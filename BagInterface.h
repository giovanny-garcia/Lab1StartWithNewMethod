...
public:
	...
	// Checks how many values are in array. Then compares each
	// entry to find the larger value between the saved entry and the one in the current index
	// before going on to compare the next entry.
	// Returns: The largest entry
	virtual ItemType findLargest() const = 0;

	// Checks how many values are in array. Then compares each
	// entry to find the smaller value between the saved entry and the one in the current index
	// before going on to compare the next entry.
